...
class GraphRule:
    def __init__(self, args, dataset):
        ...
        self.save = f'./EXPS/{args.dataset}/{args.percent}-J{args.rule_len}_hc{args.hc_thr}-sc{args.sc_thr}_{os.uname().nodename}_{time.strftime(r"%Y%m%d-%H:%M")}'
        os.mkdir(self.save)
    ...
    def qCalConf(self, mat, rule_set):
        ...
        for rule in tqdm(rule_set, desc=f"{self.epoch}-calcConfidence", ncols=60, bar_format=self.bar_format, leave=False):
            ...
            result = self.getMat(mat, rule[0]).coalesce()
            ...
            if num_result > 0:
                num_true = result * self.getMat(mat, rule[-1]).bool().float()
    ...
    def getNewTriple(self, mat, truthSet, owl):
        newTriples = []
        rightCnt = 0
        for head in range(len(mat) - 1):
            mat[head] = mat[head].coalesce()
            ...
            for h, t, w in zip(ind[0].tolist(), ind[1].tolist(), val.tolist()):
                if owl and ((h,t) not in self.data.htPair[truthSet]):
                    continue
        ...
        for w, corr, _ in newTriples:
            if corr:
                ...
                rank += 1
        assert len(allRanks) == rightCnt
        rightCnt = max(0.1, rightCnt)
        message = "{"
        message += '"split":"{}", "owl":"{}", "label":"J{}-{}-{}", "add":{}, "true":{}, "MR":{}, "MMR":{}'.format(
                truthSet, owl, self.args.rule_len, self.args.hc_thr, self.args.sc_thr,
                len(newTriples), int(rightCnt), sum(allRanks)/rightCnt, sum([1/x for x in allRanks])/rightCnt)
        message += "}\n"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-sc_thr', type=float, required=True)
    parser.add_argument('-hc_thr', type=float, required=True)
    ...
    model.getNewTriple(model.mat2, 'valid', True)
    model.getNewTriple(model.mat2, 'valid', False)
    model.getNewTriple(model.mat2, 'test', True)
    model.getNewTriple(model.mat2, 'test', False)
