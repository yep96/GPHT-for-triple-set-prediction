...
class DataBase:
    def __init__(self, para: object) -> None:
        ...
        if os.path.exists(dumpedFile):
            ...
            for i in range(50):
                desc = self.extractSubgraph(para.subgraph, para.dataset)
                print(i, desc)
                if bestMetric[0] > desc[0]:
                    bestMetric = desc
    ...
    def cntParts(self):
        ...
        self.parts.sort(key=lambda p:len(p))
        print(f"{len(self.parts)=}   {[len(x) for x in self.parts]=}")
    ...
    def extractSubgraph(self, hops, dataset):
        ...
        unExpandEnts = set(range(self.num_ent))
        ...
        while len(choices) != 0:
            ...
            for ent in choice:
                if degreeAve/2 <= degree[ent] <= 2*degreeAve:
                    break
            else:
                ent = choice[0]
            ...
            if len(subgraphEnts[ent][hops+1])<degreeAve and len(subgraphEnts[ent][3]) < (degreeAve if hops==3 else degreeAve/2):
                unExpandEnts |= expend
        ...
        subgraph = {root:subs[0] for root, subs in subgraphEnts.items()}
        subgraph.update({k:v for k,v in smallSubgraphEnts.items() if len(v)!=0})
        self.subgraph = dict()
...
class HeadDataSet(Dataset):
    def __init__(self, database: DataBase, mode: str, para: object) -> None:
        ...
        self.mode = mode
        if mode == 'train':
            self.heads = [h for h in self.subgraph.keys() if self.subgraph[h].shape[0] >= 1/self.topred]
        else:
    ...
    def __getitem__(self, head):
        triples = self.subgraph[head]
        idx = torch.randperm(triples.shape[0])
        allTriples = triples[idx]
        if self.mode == 'train':
            triples = allTriples[:triples.shape[0]*4//5]

        edge_index = torch.cat([triples[:, [0, 2]], triples[:, [2, 0]]], dim=0).t()
        edge_type = torch.cat([triples[:, 1], triples[:, 1]+self.num_rel], dim=0)

        ents, rels = edge_index[0, :].unique(), edge_type.unique()

        perdHTs = torch.cat([ents.view(-1, 1).repeat(1, len(ents)).reshape(-1, 1),
                             ents.view(-1, 1).repeat(len(ents), 1)], dim=-1)

        if self.mode == 'train':
            perdHTsLab = (allTriples[:, [0, 2]].unsqueeze(0) == perdHTs.unsqueeze(1)).all(dim=-1).any(dim=-1)
            subLab = (triples[:, [0, 2]].unsqueeze(0) == perdHTs.unsqueeze(1)).all(dim=-1).any(dim=-1)
