...
class GNNComplete(nn.Module):
    def __init__(self, database: DataBase, para) -> None:
        ...
        self.score_func = para.score_func.lower()
        ...
        self.kgeloss = {'transe': KGELoss.transe, 'rotate': KGELoss.rotate, 'hake': KGELoss.hake}[self.score_func]
    ...
    def forward(self, edge_index, edge_type, toTest, mode, negEnts=None, triples=None):
        all_ent_emb, all_rel_emb = self.gnn.forward_base(None, None, self.drop, self.drop, edge_index, edge_type)
        fetch = {}
        ...
        if (not self.para.pretrain) and (type(toTest) != type(None)):
            ...
            mod_relation = (mod_relation + bias_relation) / (1 - bias_relation)
